...
public abstract class HistoryTradeDao extends BaseDao<HistoryTrade> {
    ...
    @Query("SELECT DISTINCT symbol FROM historytrade WHERE time BETWEEN :startTime AND :endTime ORDER BY time ASC")
    public abstract List<String> getTradedPairsForDay(long startTime, long endTime);

    @Query("SELECT DISTINCT * FROM historytrade WHERE time BETWEEN :startTime AND :endTime AND symbol LIKE :name ORDER BY time ASC")
